...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.courierService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<Courier[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/courier'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Courier) {
        return item.id;
    }
